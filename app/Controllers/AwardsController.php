...
final class AwardsController
{
    ...
    public function nominations(Base $f3, array $args = []): void
    {
        ...
        $this->renderer->render(
            '/views/settings/nominations/nominations.htm',
            '/views/settings/layout.htm',
            [
...
//        }
    ...
    public function nomineeNominatorContact(Base $f3, array $args = []): void
    {
        ...
        $nominator_nominee = $args['setting'] == 'nominator' ? 'nominators' : 'nominees';
        $data = $this->awards->fetchNominatorNomineeContactInformation($awardSlug, $nominator_nominee);
        if($data){
            $f3->set('data', $data[0]);
            $f3->set('update', true);
        }
        ...
        try {
            if ($verb === 'GET') {
                ...
                $this->renderer->render(
                    '/views/settings/forms/nominator_nominee_contact_form.htm',
                    '/views/settings/layout.htm',
                    [
                        'PAGE_TITLE' => 'Contact Information for the '.ucfirst($nominator_nominee),
                        'action' => $path,
                        'setting' => $args['setting'],
                        'nominator_nominee' => $nominator_nominee
                    ]);
                return;
            }

            $post = (array)$f3->clean($f3->get('POST') ?? []);
            CsrfGuard::validate($f3, $post['token']);
            $insertId = $this->awards->createUpdateNominatorNomineeContact($awardSlug, $nominator_nominee, $post);
            return;
    ...
    public function sections(Base $f3, array $args = []): void
    {
        ...
        $render = function (string $view, array $data = []) {
            $this->renderer->render($view, '/views/settings/layout.htm', $data);
        };
        try {
            switch ($action) {
                case 'store':
                    if ($verb === 'GET') {
                        $f3->clear('SESSION.csrf_token');
                        CsrfGuard::issueToken($f3);
                        $render('/views/settings/forms/sections_form.htm', [
                ...
                default:
                    ...
                    $render('/views/settings/questions/sections.htm', [
                        'PAGE_TITLE' => 'Sections',
                        'sections' => $sections,
                        'setting' => '',
                    ]);
            ...
            switch ($action) {
                case 'store':
                    if ($verb === 'GET') {
                        $f3->clear('SESSION.csrf_token');
                        CsrfGuard::issueToken($f3);
                        $render('/views/settings/forms/question_form.htm', [
                ...
                case 'judging':
                    ...
                    if ($verb === 'GET') {
                        CsrfGuard::issueToken($f3);
                        $render('/views/settings/forms/questions_judging_forms.htm', [
                ...
                default:
                    ...
                    $render('/views/settings/questions/questions.htm', [
    ...
    {
        $awardSlug = (string)($args['awardSlug'] ?? $args['award_slug'] ?? $f3->get('SESSION.awardSlug') ?? '');
        $type = (string)($args['type'] ?? $args['type'] ??  '');
            ...
            switch ($action) {
                ...
                case 'store':
                    if ($verb === 'GET') {
                        $f3->clear('SESSION.csrf_token');
                        CsrfGuard::issueToken($f3);
                        $render('/views/settings/forms/followup_form.htm', [
